...
CREATE UNIQUE INDEX [IX_ClinicSettings_DoctorId_Key] ON [ClinicSettings] ([DoctorId], [Key]) WHERE [DoctorId] IS NOT NULL;
...
CREATE TABLE [DoctorLicenses] (
    ...
    [DeletedAt] datetime2 NULL,
    [DeletedBy] nvarchar(max) NULL,
    CONSTRAINT [PK_DoctorLicenses] PRIMARY KEY ([Id]),
    CONSTRAINT [FK_DoctorLicenses_Doctors_DoctorId] FOREIGN KEY ([DoctorId]) REFERENCES [Doctors] ([Id]) ON DELETE CASCADE
);

CREATE INDEX [IX_DoctorLicenses_DoctorId] ON [DoctorLicenses] ([DoctorId]);

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20260803191921_AddDoctorLicenses', N'10.0.10');
